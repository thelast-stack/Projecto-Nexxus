...
class NexxusAppIntegrationTests(unittest.TestCase):
    ...
    def add_catalog_resource(self, **overrides) -> str:
        ...
        return next(iter(app_module.RESOURCE_CATALOG))

    def plan_prepare_start(self, demand_id: str) -> None:
        resource_id = self.add_catalog_resource()
        self.post("/action", {"id": demand_id, "action": "plan", "resource_id": resource_id})
    ...
    def test_planning_rejects_unknown_resource_id(self):
        demand_id = self.create_demand()
        with self.assertRaises(urllib.error.HTTPError) as ctx:
            self.post("/action", {"id": demand_id, "action": "plan", "resource_id": "R-999"})
        self.assertEqual(ctx.exception.code, 400)
    ...
    def test_custom_stage_names_are_used(self):
        ...
        self.post(
            "/action",
            {"id": demand_id, "action": "plan", "resource_id": resource_id, "stages": "recolha, transporte, entrega, descarga"},
        )
        stage_names = [s.name for s in app_module.OPERATIONS[demand_id].stages]
    ...
    def test_event_location_and_quantity_are_recorded(self):
        ...
        self.post(
            "/action",
            {"id": demand_id, "action": "event", "description": "Saiu da origem", "location": "Viana", "quantity": "20"},
        )
        event = app_module.OPERATIONS[demand_id].events[-1]
        self.assertEqual(event.location, "Viana")
        self.assertEqual(event.quantity, 20.0)

        html = urllib.request.urlopen(self.url(f"/operation?id={demand_id}")).read().decode()
